...
September was the annual time for the [Driving Simulation Conference](https://dsc2021.org/) (DSC). I was really looking forward to Epic's keynote (Sébastien Lozé), but it was mostly rehashed content from [Unreal Build: Automotive 2021](https://www.unrealengine.com/en-US/events/unreal-build-automotive-2021), with some random Metaverse added on top. So no real news on the Epic front.
...
In my last example, I tried using Unreal's new [Control Rig](https://docs.unrealengine.com/4.27/en-US/AnimatingObjects/SkeletalMeshAnimation/ControlRig/) plugin, which turned out to be really useful for what I was doing. Not sure if we'll use that excavator in the end, but it was fun to create!

# PLEV

If you live in a relatively large city, chances are that you've encountered Personal Light Electric Vehicle (PLEV). You probably didn't know, as I, that it was called that way. This somewhat new trend is the focus of one of our next project, called NEWMOB. In it, we'll mostly focus on electric scooter, which I've already talked about in our [last article](/anatomy-of-a-scenario).

<video width="720" height="480" controls>
  <source type="video/mp4" src="https://cdn.discordapp.com/attachments/725724080526852126/884092494835503134/unicycle_rider.mp4.mp4">
</video>
...
We're really excited about the opportunities our new platform gives us to study those emerging fields. We're just getting started!
...
I've contribued to some Unreal-related open-source projets in the past months, so I want to give a quick shout-out to them.

The [LabStreamingLayer UE4 plugin](https://github.com/labstreaminglayer/plugin-UE4) is now available on the [Marketplace](https://www.unrealengine.com/marketplace/en-US/product/labstreaminglayer-plugin)! I co-maintain the plugin (with the amazing [Chadwick Boulay](https://github.com/cboulay), and it was my first Marketplace product, so that's a great step.

The [SDL2 UE4 plugin](https://github.com/JaydenMaalouf/JoystickPlugin) has a new maintainer (thanks [JaydenMaalouf](https://github.com/JaydenMaalouf)!), which fixed a lot of issues and added some very nice features. I really didn't do much here, but I want to mention the project nonetheless.

The [Git UE4 plugin](https://github.com/ProjectBorealis/UEGitPlugin) also has a new maintainer in [mastercoms](https://github.com/mastercoms), which also greatly improved on the existing plugin. My [fork](https://github.com/brifsttar/UE4GitPlugin) has some work on submodule support (history and diff).

Finally, I can't not mention [esmini](https://github.com/esmini/esmini), without which none of my work would be possible.

# Behind the scenes

Before ending this, a few words about some "behind the scenes" work that isn't shiny, but still important.

I've migrated our [Passation](/scenarios#beyond-scenarios-passation) feature-set to a [Subsystem](https://docs.unrealengine.com/4.27/en-US/ProgrammingAndScripting/Subsystems/), which was the best way of learning about those. This will allow for cleaner and better future extension of the whole system.
