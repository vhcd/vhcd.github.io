...
But the main change is that I finally moved the whole codebase to Unreal, making everything a `UObject`. Initially, the Virtual Driver was in its own library, because at the time we had our own engine, and didn't know whether we'd move to Unity or Unreal. That's long gone, and now, having all of those features properly integrated in the engine means more opportunities to add new features, and easier debugging.
