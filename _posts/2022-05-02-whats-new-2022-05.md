...
So few things to note, though. First, the [Focus Area](https://varjo.com/use-center/get-to-know-your-headset/getting-the-perfect-image-quality/) is slightly below the horizon, meaning if you look straight ahead, that screen will only cover a small fraction of your [fovea](https://en.wikipedia.org/wiki/Fovea_centralis). This is intentional, as this feature was added for extra accuracy on cockpits, so when the gaze is slightly lower. I've also had some trouble getting some features working ([Lab Tools](https://developer.varjo.com/lab-tools), [Depth Occlusion](https://developer.varjo.com/docs/unreal/depth-occlusion-with-unreal)), but support is really helpful.
...
![vd_objects.png]({{site.baseurl}}/_posts/vd_objects.png)
